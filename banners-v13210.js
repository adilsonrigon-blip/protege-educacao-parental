(() => {
  ...
  async function initCarousel(){
    ...
    const start=()=>{stop();if(rows.length>1&&!document.hidden)timer=setTimeout(()=>{go(current+1);start();},6000);};
    ...
    let touchX=null;viewport?.addEventListener('touchstart',e=>{touchX=e.changedTouches[0]?.clientX??null;stop();},{passive:true});viewport?.addEventListener('touchend',e=>{if(touchX===null){start();return;}const dx=(e.changedTouches[0]?.clientX??touchX)-touchX;if(Math.abs(dx)>45)go(current+(dx<0?1:-1));touchX=null;start();},{passive:true});
