...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if(action==='approve-pre-cadastro'){
      ...
      const {data:professional,error:pError}=await admin.from('profissionais').insert({auth_user_id:authId,nome:pre.nome,email:pre.email,telefone:pre.celular,especialidade:pre.especialidade,cpf:digits(pre.cpf),cep:pre.cep||null,logradouro:pre.logradouro||null,numero:pre.numero||null,complemento:pre.complemento||null,bairro:pre.bairro||null,cidade:pre.cidade||null,estado:pre.estado||null,status,updated_at:new Date().toISOString()}).select('*').single();if(pError){await admin.auth.admin.deleteUser(authId);return json({error:pError.message},400)}
