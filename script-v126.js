...
const ProtegeApp = (() => {
  ...
  async function loadAdolescents() {
    if (!configured) return [];
    const { data, error } = await db.from('cadastros_adolescentes').select('id,nome,data_nascimento,cpf,celular,email,created_at').order('nome',{ascending:true});
    if (error) throw error;
    return data || [];
  }
  ...
  async function loadAttendancesPage({page=1,pageSize=25,status=''}={}) {
    if (!configured) return {rows:[],count:0,page,pageSize};
    let query=db.from('atendimentos').select('*, profissionais(nome,ong_id,ongs(id,nome)), filhos(nome), familias(responsavel1,responsavel2,ong_id,ongs(id,nome)), cadastros_adolescentes(id,nome,data_nascimento,celular,email)',{count:'exact'}).order('data_hora',{ascending:false});
  ...
  async function loadAttendances(familyId=null, limit=100) {
    if (!configured) return [];
    let q = db.from('atendimentos').select('*, profissionais(nome,ong_id,ongs(id,nome)), filhos(nome), familias(responsavel1,responsavel2,ong_id,ongs(id,nome)), cadastros_adolescentes(id,nome,data_nascimento,celular,email)').order('data_hora',{ascending:false}).limit(limit);
  ...
  async function loadSchedules(start=null,end=null) {
    if (!configured) return [];
    let q=db.from('agenda').select('*, profissionais(nome,ong_id,ongs(id,nome)), filhos(nome), familias(responsavel1,responsavel2,ong_id,ongs(id,nome)), cadastros_adolescentes(id,nome,data_nascimento,celular,email)').order('data_inicio',{ascending:true});
  ...
  async function getSchedule(id) {
    ...
    const {data,error}=await db.from('agenda').select('*, profissionais(nome,ong_id,ongs(id,nome)), filhos(nome), familias(responsavel1,responsavel2,ong_id,ongs(id,nome)), cadastros_adolescentes(id,nome,data_nascimento,celular,email)').eq('id',id).single();
  ...
  return {configured,db,esc,statusLabel,formatDate,onlyDigits,uid,getLocalLeads,setLocalLeads,currentSession,requireAuth,loadLeads,updateLead,loadOngs,saveOng,updateOng,loadFamilies,loadAdolescents,loadFamiliesPage,updateFamilyOng,updateFamily,updateChild,saveChild,convertLead,loadProfessionals,saveProfessional,manageProfessionalAccess,loadProfessionalsWithAccess,createProfessionalWithAccess,updateProfessionalAccess,resetProfessionalPassword,updateProfessionalOng,updateProfessionalDetails,loadAttendances,loadAttendancesPage,saveAttendance,loadAttendanceEvolutions,saveAttendanceEvolution,loadSchedules,getSchedule,saveSchedule,updateSchedule,countNewLeads};
})();
...
async function initAttendancesPage(){
  ...
  function familyName(a){const f=a.familias||{};return [f.responsavel1,f.responsavel2].filter(Boolean).join(' / ')||a.cadastros_adolescentes?.nome||'—';}
  function attendanceSubjectLabel(a){return a.adolescente_id?'Adolescente':'Família';}
  function memberName(a){return a?.dados?.membro_atendido||a?.filhos?.nome||a?.cadastros_adolescentes?.nome||(a.tipo_alvo==='familia'?'Família completa':a.tipo_alvo==='responsaveis'?'Responsável(is)':a.tipo_alvo==='adolescente'?'Adolescente':'—');}
  ...
  function render(){const rows=filtered();tbody.innerHTML=rows.length?rows.map(a=>`<tr><td>${ProtegeApp.formatDate(a.data_hora)}</td><td><b>${ProtegeApp.esc(familyName(a))}</b></td><td>${ProtegeApp.esc(memberName(a))}</td><td>${ProtegeApp.esc(a.profissionais?.nome||'—')}</td><td>${ProtegeApp.esc(a.familias?.ongs?.nome||(a.adolescente_id?'Não se aplica':'—'))}</td><td><span class="status-pill ${statusClass(a.status)}">${ProtegeApp.esc(statusText(a.status))}</span></td><td><div class="attendance-row-actions"><button class="small-btn view-attendance" data-id="${ProtegeApp.esc(a.id)}">Abrir</button><button class="small-btn add-evolution" data-id="${ProtegeApp.esc(a.id)}">+ Evolução</button></div></td></tr>`).join(''):'<tr><td colspan="7" class="empty-state">Nenhum atendimento encontrado.</td></tr>';}
  ...
  async function openDetail(id,focusEvolution=false){
    ...
    document.getElementById('attendanceDetailSummary').innerHTML=`<div class="detail-card"><span>${attendanceSubjectLabel(a)}</span><b>${ProtegeApp.esc(familyName(a))}</b></div><div class="detail-card"><span>Atendimento para</span><b>${ProtegeApp.esc(memberName(a))}</b></div><div class="detail-card"><span>Profissional</span><b>${ProtegeApp.esc(a.profissionais?.nome||'—')}</b></div><div class="detail-card"><span>ONG</span><b>${ProtegeApp.esc(a.familias?.ongs?.nome||(a.adolescente_id?'Não se aplica':'Nenhuma'))}</b></div><div class="detail-card"><span>Status</span><b>${ProtegeApp.esc(statusText(a.status))}</b></div>`;
    const dados=a.dados||{};
    ...
    const familyLink=document.getElementById('attendanceFamilyLink');if(a.familia_id){familyLink.hidden=false;familyLink.href=`familias.html?id=${encodeURIComponent(a.familia_id)}`;familyLink.textContent='Abrir família';}else{familyLink.hidden=true;}
    ...
    try{
      ...
      const sections=`
        <section class="print-section"><h2>Identificação do atendimento</h2><div class="print-grid">
          <div class="print-card"><span>${attendanceSubjectLabel(a)}</span><b>${ProtegeApp.esc(familyName(a))}</b></div>
          ...
          <div class="print-card"><span>ONG</span><b>${ProtegeApp.esc(a.familias?.ongs?.nome||(a.adolescente_id?'Não se aplica':'Nenhuma'))}</b></div>
...
async function initReportsPage(){
  ...
  const familyNameFrom = r => {
    const f=r.familias||{};
    return [f.responsavel1,f.responsavel2].filter(Boolean).join(' / ')||r.cadastros_adolescentes?.nome||'—';
  };
  const attendanceTarget = a => a?.dados?.membro_atendido||a?.filhos?.nome||a?.cadastros_adolescentes?.nome||(a.tipo_alvo==='familia'?'Família completa':a.tipo_alvo==='responsaveis'?'Responsável(is)':a.tipo_alvo==='adolescente'?'Adolescente':'—');
...
async function initAttendanceWizard() {
  ...
  let current=1; const total=10; let families=[]; let adolescents=[]; let professionals=[];
  const familySel=document.getElementById('attendanceFamily'),adolescentSel=document.getElementById('attendanceAdolescent'),professionalSel=document.getElementById('attendanceProfessional'),targetSel=document.getElementById('attendanceTarget'),dateInput=document.getElementById('attendanceDateTime'),statusSel=document.getElementById('attendanceStatus');
  ...
  function selectedAdolescent(){return adolescents.find(a=>a.id===adolescentSel.value);}
  function lockExclusive(source){
    if(source==='family'&&familySel.value){adolescentSel.value='';adolescentSel.disabled=true;}else if(source==='family'){adolescentSel.disabled=false;}
    if(source==='adolescent'&&adolescentSel.value){familySel.value='';familySel.disabled=true;}else if(source==='adolescent'){familySel.disabled=false;}
    if(!familySel.value&&!adolescentSel.value){familySel.disabled=false;adolescentSel.disabled=false;}
  }
  function refreshSubject(){
    const f=selectedFamily(),a=selectedAdolescent();
    if(f){
      const ch=f.filhos||[],members=[];
    ...
    if(a){
      targetSel.innerHTML=`<option value="adolescente:${ProtegeApp.esc(a.id)}">${ProtegeApp.esc(a.nome)} · Adolescente</option>`;
      document.getElementById('attendanceOngInfo').textContent='Não se aplica ao cadastro de adolescente';
      const nasc=a.data_nascimento?new Date(a.data_nascimento+'T12:00:00').toLocaleDateString('pt-BR'):'—';
      document.getElementById('attendanceFamilySummary').innerHTML=`<span>Adolescente selecionado</span><b>${ProtegeApp.esc(a.nome)}</b><small>Nascimento: ${ProtegeApp.esc(nasc)} · ${ProtegeApp.esc(a.celular||'')} · ${ProtegeApp.esc(a.email||'')}</small>`;
      return;
    }
    targetSel.innerHTML='<option value="">Selecione uma família ou adolescente</option>';
    document.getElementById('attendanceFamilySummary').innerHTML='Selecione uma família ou adolescente acima.';
    document.getElementById('attendanceOngInfo').textContent='Nenhuma família selecionada';
  }
  function selectedTarget(){
    const raw=targetSel.value||'',[kind,id]=raw.split(':'),f=selectedFamily(),a=selectedAdolescent();
    if(kind==='adolescente'&&a&&a.id===id)return {tipo_alvo:'adolescente',filho_id:null,adolescente_id:a.id,nome:a.nome};
    if(!f||!kind)return null;
    if(kind==='familia')return {tipo_alvo:'familia',filho_id:null,adolescente_id:null,nome:`Família ${f.responsavel1}${f.responsavel2?' / '+f.responsavel2:''}`};
    if(kind==='responsavel1')return {tipo_alvo:'responsaveis',filho_id:null,adolescente_id:null,nome:f.responsavel1||'Responsável'};
    if(kind==='responsavel2')return {tipo_alvo:'responsaveis',filho_id:null,adolescente_id:null,nome:f.responsavel2||'Responsável'};
    if(kind==='filho'){const c=(f.filhos||[]).find(x=>x.id===id);return {tipo_alvo:'filho',filho_id:id,adolescente_id:null,nome:c?.nome||'Filho(a)'};}
    return null;
  }
  try{
    [families,adolescents,professionals]=await Promise.all([ProtegeApp.loadFamilies(),ProtegeApp.loadAdolescents(),ProtegeApp.loadProfessionals()]);
    ...
    adolescentSel.innerHTML='<option value="">Selecione o adolescente</option>'+adolescents.map(a=>`<option value="${ProtegeApp.esc(a.id)}">${ProtegeApp.esc(a.nome)}</option>`).join('');
    ...
    const params=new URLSearchParams(location.search),qid=params.get('familia'),aid=params.get('adolescente'),agendaId=params.get('agenda');
    if(qid&&families.some(f=>f.id===qid)){familySel.value=qid;lockExclusive('family');}
    if(aid&&adolescents.some(a=>a.id===aid)){adolescentSel.value=aid;lockExclusive('adolescent');}
    const d=new Date();d.setMinutes(d.getMinutes()-d.getTimezoneOffset());dateInput.value=d.toISOString().slice(0,16);refreshSubject();
    if(agendaId){try{const ag=await ProtegeApp.getSchedule(agendaId);if(ag){if(ag.adolescente_id){adolescentSel.value=ag.adolescente_id;lockExclusive('adolescent');}else{familySel.value=ag.familia_id||'';lockExclusive('family');}refreshSubject();professionalSel.value=ag.profissional_id||'';dateInput.value=(()=>{const x=new Date(ag.data_inicio);x.setMinutes(x.getMinutes()-x.getTimezoneOffset());return x.toISOString().slice(0,16)})();let tv=ag.tipo_alvo==='adolescente'&&ag.adolescente_id?`adolescente:${ag.adolescente_id}`:ag.tipo_alvo==='familia'?`familia:${ag.familia_id}`:ag.tipo_alvo==='filho'&&ag.filho_id?`filho:${ag.filho_id}`:'';if(ag.tipo_alvo==='responsavel'){const f=selectedFamily();tv=ag.atendimento_para===f?.responsavel2?`responsavel2:${ag.familia_id}`:`responsavel1:${ag.familia_id}`;}targetSel.value=tv;statusSel.value='realizado';document.getElementById('attendanceSetupMessage').innerHTML=`Atendimento iniciado a partir da agenda para <b>${ProtegeApp.esc(ag.atendimento_para)}</b>.`;}}catch(err){console.error(err);}}
  ...
  }catch(err){console.error(err);document.getElementById('attendanceSetupMessage').textContent='Não foi possível carregar famílias, adolescentes ou profissionais. Confirme se o SQL da V13.18.0 foi executado.';}
  familySel.addEventListener('change',()=>{lockExclusive('family');refreshSubject();});
  adolescentSel.addEventListener('change',()=>{lockExclusive('adolescent');refreshSubject();});
  ...
  form.addEventListener('submit',async e=>{e.preventDefault();message.textContent='';const hasFamily=!!familySel.value,hasAdolescent=!!adolescentSel.value;if(hasFamily===hasAdolescent||!professionalSel.value||!targetSel.value||!dateInput.value){message.textContent='Selecione somente uma opção: Família ou Adolescente, além do profissional, pessoa atendida e data/hora.';return;}const alvo=selectedTarget();if(!alvo){message.textContent='Selecione quem será atendido.';return;}save.disabled=true;save.textContent='Salvando...';try{const dados=formDataToObject(form);dados.membro_atendido=alvo.nome;dados.atendimento_para=targetSel.value;const row=await ProtegeApp.saveAttendance({familia_id:hasFamily?familySel.value:null,adolescente_id:hasAdolescent?adolescentSel.value:null,profissional_id:professionalSel.value,filho_id:alvo.filho_id,tipo_alvo:alvo.tipo_alvo,data_hora:new Date(dateInput.value).toISOString(),status:statusSel.value,etapa_atual:10,dados,observacoes:String(dados.observacoes||'').trim()||null});const agendaId=new URLSearchParams(location.search).get('agenda');if(agendaId)await ProtegeApp.updateSchedule(agendaId,{status:'realizado',atendimento_id:row.id});const link=row.familia_id?` <a href="familias.html?id=${encodeURIComponent(row.familia_id)}"><b>Abrir ficha da família</b></a>.`:'';message.innerHTML=`Atendimento salvo com sucesso para <b>${ProtegeApp.esc(alvo.nome)}</b>.${link}`;message.scrollIntoView({behavior:'smooth'});}catch(err){console.error(err);message.textContent=`Não foi possível salvar: ${err?.message||'erro inesperado'}`;}finally{save.disabled=false;save.textContent='Salvar atendimento';}});
  render();
}


async function initAgendaPage(){
  const dateInput=document.getElementById('agendaDate'),list=document.getElementById('agendaDayList'),professionalFilter=document.getElementById('agendaProfessionalFilter'),ongFilter=document.getElementById('agendaOngFilter');
  ...
  const familySel=document.getElementById('scheduleFamily'),adolescentSel=document.getElementById('scheduleAdolescent'),targetSel=document.getElementById('scheduleTarget'),professionalSel=document.getElementById('scheduleProfessional');
  let items=[],families=[],adolescents=[],professionals=[],ongs=[],editingId=null;
  const localKey=d=>{const x=new Date(d);return `${x.getFullYear()}-${String(x.getMonth()+1).padStart(2,'0')}-${String(x.getDate()).padStart(2,'0')}`;};
  ...
  function familyName(a){return a.familias?[a.familias.responsavel1,a.familias.responsavel2].filter(Boolean).join(' / '):(a.cadastros_adolescentes?.nome||'—');}
  function subjectKind(a){return a.adolescente_id?'Adolescente':'Família';}
  ...
  function selectedFamily(){return families.find(f=>f.id===familySel.value);}
  function selectedAdolescent(){return adolescents.find(a=>a.id===adolescentSel.value);}
  function lockExclusive(source){if(source==='family'&&familySel.value){adolescentSel.value='';adolescentSel.disabled=true;}else if(source==='family'){adolescentSel.disabled=false;}if(source==='adolescent'&&adolescentSel.value){familySel.value='';familySel.disabled=true;}else if(source==='adolescent'){familySel.disabled=false;}if(!familySel.value&&!adolescentSel.value){familySel.disabled=false;adolescentSel.disabled=false;}}
  function targetOptions(f,a){if(a)return `<option value="adolescente:${ProtegeApp.esc(a.id)}">${ProtegeApp.esc(a.nome)} · Adolescente</option>`;if(!f)return '<option value="">Selecione uma família ou adolescente</option>';const out=[`<option value="familia:${ProtegeApp.esc(f.id)}">Família completa</option>`];if(f.responsavel1)out.push(`<option value="responsavel1:${ProtegeApp.esc(f.id)}">${ProtegeApp.esc(f.responsavel1)} · Responsável</option>`);if(f.responsavel2)out.push(`<option value="responsavel2:${ProtegeApp.esc(f.id)}">${ProtegeApp.esc(f.responsavel2)} · Responsável</option>`);(f.filhos||[]).forEach(c=>out.push(`<option value="filho:${ProtegeApp.esc(c.id)}">${ProtegeApp.esc(c.nome)} · Filho(a)${c.idade!=null?' · '+c.idade+' ano(s)':''}</option>`));return '<option value="">Selecione quem será atendido</option>'+out.join('');}
  function selectedTarget(){const f=selectedFamily(),a=selectedAdolescent(),[kind,id]=(targetSel.value||'').split(':');if(kind==='adolescente'&&a&&a.id===id)return {tipo_alvo:'adolescente',filho_id:null,adolescente_id:a.id,nome:a.nome};if(!f||!kind)return null;if(kind==='familia')return {tipo_alvo:'familia',filho_id:null,adolescente_id:null,nome:'Família completa'};if(kind==='responsavel1')return {tipo_alvo:'responsavel',filho_id:null,adolescente_id:null,nome:f.responsavel1};if(kind==='responsavel2')return {tipo_alvo:'responsavel',filho_id:null,adolescente_id:null,nome:f.responsavel2};if(kind==='filho'){const c=(f.filhos||[]).find(x=>x.id===id);return {tipo_alvo:'filho',filho_id:id,adolescente_id:null,nome:c?.nome||'Filho(a)'};}return null;}
  function refreshTarget(){const f=selectedFamily(),a=selectedAdolescent();targetSel.innerHTML=targetOptions(f,a);document.getElementById('scheduleOngInfo').textContent=f?(f.ongs?.nome||'Nenhuma ONG vinculada'):(a?'Não se aplica ao cadastro de adolescente':'Nenhuma família selecionada');}
  ...
  form.addEventListener('submit',async e=>{e.preventDefault();const t=selectedTarget(),msg=document.getElementById('scheduleMessage');const hasFamily=!!familySel.value,hasAdolescent=!!adolescentSel.value;if(hasFamily===hasAdolescent||!professionalSel.value||!t){msg.textContent='Selecione somente uma opção: Família ou Adolescente, além da pessoa atendida e profissional.';return;}syncHiddenDateTimes();const start=document.getElementById('scheduleStart').value,end=document.getElementById('scheduleEnd').value;if(!start){msg.textContent='Informe data e hora de início.';return;}if(end&&new Date(end)<=new Date(start)){msg.textContent='O horário de término deve ser posterior ao início.';return;}const body={familia_id:hasFamily?familySel.value:null,adolescente_id:hasAdolescent?adolescentSel.value:null,filho_id:t.filho_id,profissional_id:professionalSel.value,atendimento_para:t.nome,tipo_alvo:t.tipo_alvo,tipo:document.getElementById('scheduleType').value,data_inicio:new Date(start).toISOString(),data_fim:end?new Date(end).toISOString():null,status:document.getElementById('scheduleStatus').value,observacoes:document.getElementById('scheduleNotes').value.trim()||null};try{document.getElementById('saveScheduleButton').disabled=true;if(editingId)await ProtegeApp.updateSchedule(editingId,body);else await ProtegeApp.saveSchedule(body);msg.textContent='Agendamento salvo com sucesso.';await reload();setTimeout(()=>closeScheduleModal(),350);}catch(err){console.error(err);msg.textContent='Não foi possível salvar: '+(err?.message||'erro');}finally{document.getElementById('saveScheduleButton').disabled=false;}});
}
