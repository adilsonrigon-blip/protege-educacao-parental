(function () {
  ...
  async function act(action, extra={}) { if (!current) return; reviewMsg.textContent='Processando...'; try { await invoke({ action, pre_cadastro_id:current.id, observacoes_admin:notes.value.trim(), ...extra }); dialog.close(); await load(); await window.ProtegeRefreshPendingBadges?.(); } catch(e){ reviewMsg.textContent=e.message || 'Não foi possível concluir a ação.'; } }
